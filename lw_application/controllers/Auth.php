...
class Auth extends Base_Controller
{
    ...
    function login()
    {
        if ($this->tank_auth->is_logged_in()) {                                 // logged in
            ...
            $this->form_validation->set_rules('login', __('Login'), 'trim|required');
            ...
            if ($this->tank_auth->is_max_login_attempts_exceeded($login)) {
                if ($data['use_recaptcha'])
                    $this->form_validation->set_rules('recaptcha_response_field', __('Confirmation Code'), 'trim|required|callback__check_recaptcha');
                else {
                    $this->form_validation->set_rules('captcha', __('Confirmation Code'), 'trim|required|callback_check_captcha');
                }
    ...
    function register()
    {
        ...
        if ($this->tank_auth->is_logged_in()) {                                 // logged in
            ...
            if ($use_username) {
                $this->form_validation->set_rules('username', __('Username'), 'trim|required|min_length['.$this->config->item('username_min_length', 'tank_auth').']|max_length['.$this->config->item('username_max_length', 'tank_auth').']|alpha_dash');
            }
            $this->form_validation->set_rules('email', __('Email'), 'trim|required|valid_email');
            $this->form_validation->set_rules('password', __('Password'), 'trim|required|min_length['.$this->config->item('password_min_length', 'tank_auth').']|max_length['.$this->config->item('password_max_length', 'tank_auth').']|alpha_dash');
            $this->form_validation->set_rules('confirm_password', __('Confirm Password'), 'trim|required|matches[password]');
    ...
    function forgot_password()
    {
        if ($this->tank_auth->is_logged_in()) {                                 // logged in
            ...
            $this->form_validation->set_rules('login', __('Email or login'), 'trim|required');
    ...
    function reset_password()
    {
        ...
        $this->form_validation->set_rules('new_password', __('New Password'), 'trim|required|min_length['.$this->config->item('password_min_length', 'tank_auth').']|max_length['.$this->config->item('password_max_length', 'tank_auth').']|alpha_dash');
        $this->form_validation->set_rules('confirm_new_password', __('Confirm new Password'), 'trim|required|matches[new_password]');
    ...
    function change_password()
    {
        if (!$this->tank_auth->is_logged_in()) {                                // not logged in or not activated
            ...
            $this->form_validation->set_rules('old_password', __('Old Password'), 'trim|required');
            $this->form_validation->set_rules('new_password', __('New Password'), 'trim|required|min_length['.$this->config->item('password_min_length', 'tank_auth').']|max_length['.$this->config->item('password_max_length', 'tank_auth').']|alpha_dash');
            $this->form_validation->set_rules('confirm_new_password', __('Confirm new Password'), 'trim|required|matches[new_password]');
